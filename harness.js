...
var IndexedDbPersistedUiStateStore = class {
  ...
  async loadRouteHead(appKey, routeHref) {
    ...
    const routeHeads = transaction.objectStore(PERSISTED_UI_STATE_ROUTE_HEADS_STORE);
    const routeHead2 = await requestToPromise(
      routeHeads.get(buildPersistedUiRouteKey(appKey, routeHref))
    );
    await transactionToPromise(transaction);
    return routeHead2 ?? null;
  }
  ...
  async collectGarbage(now) {
    ...
    for (const routeHead2 of routeHeadRecords) {
      if (!knownSnapshotIds.has(routeHead2.snapshotId)) {
        routeHeads.delete(routeHead2.routeKey);
      }
    }
...
${nodeId}`;
  ...
  async loadRouteHeadSnapshot(routeHref, context) {
    ...
    try {
      const routeHead2 = await this.persistedUiState.loadRouteHead(window.location.origin, routeHref);
      if (routeHead2 === null) {
        return null;
      }
      return await this.loadPersistedSnapshotById(routeHead2.snapshotId, `${context} via route head`);
    } catch (error) {
...
var RoutedAppRoute = class {
  ...
  sourceRoutePath;
  publishedRoutePath;
  constructor(key = "", title = "", headTags = new Array(), entrypoint = "", wasmFile = "", shellDir = "", sourceRoutePath = "", publishedRoutePath = "") {
    this.key = key;
    this.title = title;
...
var RoutedHarnessRouteSpec = class {
  routePath;
  wasmPath;
  title;
...
function routeDef(key, title, headTags = new Array(), entrypoint = "", wasmFile = "", shellDir = "", sourceRoutePath = "", publishedRoutePath = "") {
...
// src/route-config.ts
var sourceRouteBase = "";
var routeManifest = defineRoutedAppManifest(sourceRouteBase, [
  routeDef("home", "Home", routeHead(
    "description",
    "Home page for the FUI-AS routed demo."
  )),
  routeDef("text-fonts", "Text & Fonts", routeHead(
    "description",
    "Rich text, custom fonts, and control configuration samples."
  )),
